...
def process_data():
    ...
    # 데이터 수집 및 처리 루프
    with sd.InputStream(samplerate=fs, channels=1, blocksize=buffer_size) as stream:
        start_time = time.time()
        while time.time() - start_time < duration:
            # 데이터 읽기
            new_data, _ = stream.read(buffer_size)
            ...
            for i in range(min(imfs.shape[0], 3)):  # 최대 3개의 IMF만 처리
